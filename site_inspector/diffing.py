...
def diff_graph(run_a: Dict[str, Any], run_b: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ga = run_a.get("graph") or {}
    gb = run_b.get("graph") or {}
    if not ga or not gb:
        return None

    orphans_a = set((ga.get("orphan_pages") or {}).get("urls") or [])
    orphans_b = set((gb.get("orphan_pages") or {}).get("urls") or [])
    dead_a = set((ga.get("dead_ends") or {}).get("urls") or [])
    dead_b = set((gb.get("dead_ends") or {}).get("urls") or [])

    pr_a = {e["url"]: e["score"] for e in ((ga.get("pagerank") or {}).get("top") or [])}
    pr_b = {e["url"]: e["score"] for e in ((gb.get("pagerank") or {}).get("top") or [])}
    pr_shifts = []
    for url in set(pr_a) | set(pr_b):
        delta = (pr_b.get(url) or 0.0) - (pr_a.get(url) or 0.0)
        if abs(delta) >= 0.01:
            pr_shifts.append({"url": url, "delta": round(delta, 6)})
    pr_shifts.sort(key=lambda x: -abs(x["delta"]))

    depth_a = ga.get("avg_depth")
    depth_b = gb.get("avg_depth")
    depth_delta = None
    if depth_a is not None and depth_b is not None:
        depth_delta = round(float(depth_b) - float(depth_a), 2)

    return {
        "node_count_a": ga.get("nodes"),
        "node_count_b": gb.get("nodes"),
        "edge_count_a": ga.get("edges"),
        "edge_count_b": gb.get("edges"),
        "avg_depth_a": depth_a,
        "avg_depth_b": depth_b,
        "avg_depth_delta": depth_delta,
        "orphans_added": sorted(orphans_b - orphans_a),
        "orphans_removed": sorted(orphans_a - orphans_b),
        "dead_ends_added": sorted(dead_b - dead_a),
        "dead_ends_removed": sorted(dead_a - dead_b),
        "pagerank_shifts": pr_shifts[:20],
    }
...
def render_diff_md(diff: Dict[str, Any]) -> str:
    ...
    if gd:
        ...
        lines.append(f"- Nodes: {gd.get('node_count_a')} → {gd.get('node_count_b')}")
        lines.append(f"- Edges: {gd.get('edge_count_a')} → {gd.get('edge_count_b')}")
        dd = gd.get("avg_depth_delta")
        if dd is not None:
            lines.append(f"- Avg depth: {gd.get('avg_depth_a')} → {gd.get('avg_depth_b')} (Δ {dd:+.2f})")
        oa = gd.get("orphans_added") or []
        or_ = gd.get("orphans_removed") or []
        lines.append(f"- Orphans added: **{len(oa)}**")
        for u in oa[:10]:
            lines.append(f"  - {u}")
        lines.append(f"- Orphans removed: **{len(or_)}**")
        for u in or_[:10]:
            lines.append(f"  - {u}")
        da = gd.get("dead_ends_added") or []
        dr = gd.get("dead_ends_removed") or []
        lines.append(f"- Dead ends added: **{len(da)}**")
        lines.append(f"- Dead ends removed: **{len(dr)}**")
        shifts = gd.get("pagerank_shifts") or []
        if shifts:
            lines.append(f"- PageRank shifts (top {len(shifts[:10])}):")
            for s in shifts[:10]:
                lines.append(f"  - {s['url']}  Δ={s['delta']:+.6f}")
        lines.append("")

    lines.append("---\n")
    lines.append(
        "*Generated by [Site Inspector](https://github.com/Mark2Mac/site-inspector)"
        " \u00b7 Built to audit [dedicatodesign.com](https://www.dedicatodesign.com)*\n"
    )
    return "\n".join(lines)
